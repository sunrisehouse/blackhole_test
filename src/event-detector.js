...
export class EventDetector {
  ...
  logFlagChange(flagName, value, t, message, detection) {
    this.flagChangeLog.push({
      ...
      value: {
        ...
        ts2: {
          flag: this.flagTs2,
          time: this.ts2Event.time,
          a: this.ts2Event.value,
        },
      },
      time: t,
      message,
  ...
  inputSoundData({ samples, t }) {
    if (this.isOn) {
      if (this.flagTs2) {
        const timeDiff = t - this.ts2Event.time;
        if (timeDiff > this.TR_WAITING) {
          this.flagTs2 = false;
          this.ts2Event.time = null;
          this.ts2Event.value = null;
          this.logFlagChange('flagTs2', false, t, 'flagTs2 false 로 바꿈', '');
        }
      }
      if (!this.flagTs2) {
        if (!this.flagTs1) {
          // 0.3 이상의 값이 있는지 확인하여 flagTs1 설정
  ...
  inputGyroData({ a, t }) {
    if (this.isOn) {
      ...
      if (!this.flagTs2) {
        if (this.flagTs1) {
          const timeSinceFlagTs2 = t - this.soundTs1Time; 
          if (timeSinceFlagTs2 > this.TS2_CONDITION_MAX_TIME) {
            this.resetFlags(t, `Resetting flagTs1 and flagTs2 after 2000ms timeout.`);
          } 
            ...
            if (a >= this.TR_CONDITION_MIN_VALUE) {
              ...
              this.logFlagChange('flagTs2', a, t, `Event detected at time ${t}, gyro: ${a}`, '각속도 감지');
            }
          } 
          // else {
